...
namespace DTcms.DAL
{
    ...
    public partial class users
    {
        ...
        public bool Exists(string user_name)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from " + databaseprefix + "users");
            strSql.Append(" where user_name=@user_name or mobile=@user_name ");
            SqlParameter[] parameters = {
        ...
        public bool Add(Model.users model,int IntegralRatio, List<string> IntegralCode)
        {
            List<CommandInfo> sqllist = new List<CommandInfo>();
            StringBuilder strSql1 = new StringBuilder();
            strSql1.Append("declare  @id int ");
            strSql1.Append("insert into " + databaseprefix + "users(");
            strSql1.Append("group_id,user_name,password,salt,email,nick_name,avatar,sex,birthday,telphone,mobile,qq,address,safe_question,safe_answer,amount,point,exp,status,reg_time,reg_ip)");
            strSql1.Append(" values (");
            strSql1.Append("@group_id,@user_name,@password,@salt,@email,@nick_name,@avatar,@sex,@birthday,@telphone,@mobile,@qq,@address,@safe_question,@safe_answer,@amount,@point,@exp,@status,@reg_time,@reg_ip)");
            strSql1.Append(";set @id =@@IDENTITY");
            SqlParameter[] parameters1 = {
					new SqlParameter("@group_id", SqlDbType.Int,4),
					new SqlParameter("@user_name", SqlDbType.NVarChar,100),
					new SqlParameter("@password", SqlDbType.NVarChar,100),
					...
					new SqlParameter("@reg_time", SqlDbType.DateTime),
					new SqlParameter("@reg_ip", SqlDbType.NVarChar,30)};
            parameters1[0].Value = model.group_id;
            parameters1[1].Value = model.user_name;
            parameters1[2].Value = model.password;
            parameters1[3].Value = model.salt;
            parameters1[4].Value = model.email;
            parameters1[5].Value = model.nick_name;
            parameters1[6].Value = model.avatar;
            parameters1[7].Value = model.sex;
            parameters1[8].Value = model.birthday;
            parameters1[9].Value = model.telphone;
            parameters1[10].Value = model.mobile;
            parameters1[11].Value = model.qq;
            parameters1[12].Value = model.address;
            parameters1[13].Value = model.safe_question;
            parameters1[14].Value = model.safe_answer;
            parameters1[15].Value = model.amount;
            parameters1[16].Value = model.point;
            parameters1[17].Value = model.exp;
            parameters1[18].Value = model.status;
            parameters1[19].Value = model.reg_time;
            parameters1[20].Value = model.reg_ip;
            CommandInfo cmd = new CommandInfo(strSql1.ToString(), parameters1);
            sqllist.Add(cmd);
            StringBuilder strSql2 = new StringBuilder();
            strSql2.Append(@"update dt_users set point=point+@IntegralRatio where user_name=@user_name");
            SqlParameter[] parameters2 = {
                    new SqlParameter("@IntegralRatio", SqlDbType.Int,8),
                    new SqlParameter("@user_name", SqlDbType.NVarChar,100)};
            parameters2[0].Value = IntegralRatio;
            parameters2[1].Value = model.user_name;
            cmd = new CommandInfo(strSql2.ToString(), parameters2);
            sqllist.Add(cmd);


            foreach (var Code in IntegralCode)
            {
                StringBuilder strSql3 = new StringBuilder();
                strSql3.Append("Update BarCode set state=@state where IntegralCode=@IntegralCode; ");
                SqlParameter[] parameters3 = {
                    new SqlParameter("@state", SqlDbType.Int,8),
                    new SqlParameter("@IntegralCode", SqlDbType.VarChar,50)};
                parameters3[0].Value = 4;
                parameters3[1].Value = Code;
            ...
            foreach (var Code in IntegralCode)
            {
                ...
                parameters4[2].Value = Code;
                parameters4[3].Value = model.reg_ip;
        ...
        public bool Update(Model.users model)
        {
            ...
            strSql.Append("BabyName=@BabyName,");
            strSql.Append("BabyNickName=@BabyNickName,");
            strSql.Append("BabyAge=@BabyAge");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4),
					new SqlParameter("@group_id", SqlDbType.Int,4),
					...
					new SqlParameter("@reg_time", SqlDbType.DateTime),
					new SqlParameter("@reg_ip", SqlDbType.NVarChar,30),
                    new SqlParameter("@BabyName", SqlDbType.NVarChar,100),
                    new SqlParameter("@BabyNickName", SqlDbType.NVarChar,100),
                    new SqlParameter("@BabyAge", SqlDbType.Int,4)};
            ...
            parameters[22].Value = model.BabyName;
            parameters[23].Value = model.BabyNickName;
            parameters[24].Value = model.BabyAge;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,group_id,user_name,password,salt,email,nick_name,avatar,sex,birthday,telphone,mobile,qq,address,safe_question,safe_answer,amount,point,exp,status,reg_time,reg_ip,BabyName,BabyNickName,BabyAge from " + databaseprefix + "users ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["BabyName"].ToString() != "")
                {
                    model.BabyName = ds.Tables[0].Rows[0]["BabyName"].ToString();
                }
                if (ds.Tables[0].Rows[0]["BabyNickName"].ToString() != "")
                {
                    model.BabyNickName =ds.Tables[0].Rows[0]["BabyNickName"].ToString();
                }
                if (ds.Tables[0].Rows[0]["BabyAge"].ToString() != "")
                {
                    model.BabyAge = int.Parse(ds.Tables[0].Rows[0]["BabyAge"].ToString());
                }
                return model;
            }
            else
