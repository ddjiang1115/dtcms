...
namespace DTcms.DAL
{
    public class Client
    {
        ...
        public List<Model.Client> GetList()
        {
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                list= Common.DataTableToList.List<Model.Client>(ds.Tables[0]);
            }
            return list;
        }
        public bool Delete(string ClientID)
        {
            ...
            strSql.Append(" where ClientID in (" + ClientID + ")");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select * from Client with(nolock) where ClientAccount=@ClientAccount");
            SqlParameter[] parameters = {
                    new SqlParameter("@ClientAccount", SqlDbType.NVarChar,100)};
            parameters[0].Value = ClientAccount;
